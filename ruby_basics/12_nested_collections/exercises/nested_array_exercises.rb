def blank_seating_chart(number_of_rows, seats_per_row)
  ...
  array2 = Array.new(number_of_rows) {Array.new(seats_per_row)}
  return array2 
...
def find_favorite(array_of_hash_objects)
  ...
  x = array_of_hash_objects.length - 1

  for i in (0..x)
    if array_of_hash_objects[i].values == true
      return array_of_hash_objects[i]
    end
  end
